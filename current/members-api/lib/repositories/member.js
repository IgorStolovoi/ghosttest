...
module.exports = class MemberRepository {
  ...
  async update(data, options) {
    ...
    if (memberData.company) {
      memberData.company = memberData.company.trim();
    }

    // if we change member name from admin -> it cause such problems as set it value to last_name first_name
    if (memberData.name) {
      memberData.name = memberData.name.trim();
      if (!(memberData.first_name && memberData.last_name)) {
        const [_, first_name, last_name] = /(\S+)\s+(.*)/.exec(memberData.name);
        memberData.first_name = first_name?.trim() || "";
        memberData.last_name = last_name?.trim() || "";
      }
    }

    if (memberData.first_name && memberData.last_name) {
      memberData.first_name = memberData.first_name.trim();
      memberData.last_name = memberData.last_name.trim();
      if (!memberData.name) {
  ...
  async list(options) {
    ...
    //
    //
    // TODO change (find a way not to set here created_at wrong value after member created)
    if (
      (options.filter.indexOf("created_at") === 0 &&
        options.filter.lastIndexOf("created_at") !== 0) ||
      (options.filter.indexOf("created_at") === 0 &&
        options.filter.lastIndexOf("created_at") === 0)
    ) {
      const oldFilters = options.filter.split("+");
      oldFilters.shift();
      const updatedFilters = oldFilters.join("+");
      options.filter = updatedFilters;
    }
    //
    //
    //
  ...
  async findMemberPrimaryEmailAndSocialNetwork({
    ...
    return await this._Member.findMemberPrimaryEmailAndSocialNetwork({
      email,
      linkedSocialNetworkName
    });
  }
  async getMemberLinkedSocialNetworkId({
    ...
    return await this._Member.getMemberLinkedSocialNetworkId({
      primaryEmail,
      linkedSocialNetworkName
    });
  }
  async setMemberLinkedSocialNetwork({
    ...
    return await this._Member.setMemberLinkedSocialNetwork({
      memberId,
      linkedSocialNetworkEmail,
      linkedSocialNetworkName
    });
  ...
  async updateMemberLinkedSocialNetwork({
    ...
    return await this._Member.updateMemberLinkedSocialNetwork({
      linkedSocialNetworkId,
      linkedSocialNetworkEmail,
      linkedSocialNetworkName
    });
  ...
  async getMemberIdForLinkedSocialNetworkId({ linkedSocialNetworkId }) {
    try {
      const result = await this._Member.getMemberIdForLinkedSocialNetworkId({
        linkedSocialNetworkId
      });
      return result;
    } catch (error) {
      logging.error(
        `Cannot find linked social network with id - ${linkedSocialNetworkId}, not found.`
      );
      return null;
    }
  }
  ...
  async removeMemberLinkedSocialNetwork({ linkedSocialNetworkId }) {
    try {
      const result = await this._Member.removeMemberLinkedSocialNetwork({
        linkedSocialNetworkId
      });
      return result;
    } catch (error) {
      logging.error(
        `Cannot remove member linked social network with id - ${linkedSocialNetworkId}, not found.`
      );
      return null;
    }
  }
  async getMemberLinkedSocialNetworks({ memberId }) {
    return await this._Member.getMemberLinkedSocialNetworks({
      memberId
    });
  }
};
