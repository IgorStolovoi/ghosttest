...
const { LinkedSocialNetworks } = require("./linked-social-networks");
...
const Member = ghostBookshelf.Model.extend(
  ...
  },
  {
    ...
    add(data, unfilteredOptions = {}) {
      console.log("add on signup");
      console.log(data);
      console.log(unfilteredOptions);
    ...
    // async findMemberPrimaryEmailByEmailFromLinkedSocialNetworks(
    //   email,
    //   linkedSocialNetwork,
    //   unfilteredOptions = {}
    // ) {
    //   // const raw = `select email from members where linked_auth_services like ?`;
    //   // const queryResponse = await ghostBookshelf.knex.raw(raw, [`%${email}%`]);
    //   // return queryResponse?.[0]?.[0]?.email;
    // },
    async findMemberPrimaryEmailAndSocialNetwork({
      email,
      linkedSocialNetworkName
    }) {
      const raw = `select members.email, members.primary_social_network as primarySocialNetwork, members.id as memberId,\
      linked_social_networks.id as linkedSocialNetworkId from members left join linked_social_networks \
      on linked_social_networks.member_id=members.id  where (linked_social_networks.email = ? \
      and linked_social_networks.social_network = ?) or (members.email = ? and members.primary_social_network = ?)`;
      console.log(
        ghostBookshelf.knex
          .raw(raw, [
            email,
      ...
      const queryResponse = await ghostBookshelf.knex.raw(raw, [
        email,
        linkedSocialNetworkName,
        email,
        linkedSocialNetworkName
      ]);
      console.log("queryResponse findMemberPrimaryEmailAndSocialNetwork");
      console.log(queryResponse);
      return {
        primaryEmail: queryResponse?.[0]?.[0]?.email,
        memberId: queryResponse?.[0]?.[0]?.memberId,
        primarySocialNetwork: queryResponse?.[0]?.[0]?.primarySocialNetwork,
        linkedSocialNetworkId: queryResponse?.[0]?.[0]?.linkedSocialNetworkId
      };
    },

    async getMemberLinkedSocialNetworkId({
      primaryEmail,
      linkedSocialNetworkName
    }) {
      const raw = `select linked_social_networks.id as linkedSocialNetworkId \
      from members inner join linked_social_networks \
      on linked_social_networks.member_id=members.id where members.email = ? \
      and linked_social_networks.social_network = ?`;
      console.log(
        ghostBookshelf.knex
          .raw(raw, [primaryEmail, linkedSocialNetworkName])
          .toQuery()
      );
      const queryResponse = await ghostBookshelf.knex.raw(raw, [
        primaryEmail,
      ...
      console.log("queryResponse getMemberLinkedSocialNetworkId");
      console.log(queryResponse);
      return {
        linkedSocialNetworkId: queryResponse?.[0]?.[0]?.linkedSocialNetworkId
      };
    ...
    async setMemberLinkedSocialNetwork({
      memberId,
      linkedSocialNetworkEmail,
      linkedSocialNetworkName
    }) {
      const result = await LinkedSocialNetworks.add({
        member_id: memberId,
        social_network: linkedSocialNetworkName,
        email: linkedSocialNetworkEmail
      });

      console.log("setMemberLinkedSocialNetwork result");
      console.log(result);
      return result;
    },

    async updateMemberLinkedSocialNetwork({
      linkedSocialNetworkId,
      linkedSocialNetworkEmail,
      linkedSocialNetworkName
    }) {
      const result = await LinkedSocialNetworks.edit(
        {
          social_network: linkedSocialNetworkName,
          email: linkedSocialNetworkEmail
        },
        { id: linkedSocialNetworkId }
      );

      console.log("updateMemberLinkedSocialNetwork  result");
      console.log(result);
      return result;
    },

    async removeMemberLinkedSocialNetwork({ linkedSocialNetworkId }) {
      const result = await LinkedSocialNetworks.destroy({
        id: linkedSocialNetworkId
      });

      console.log("removeMemberLinkedSocialNetwork result");
      console.log(result);
      return result;
    },

    async getMemberIdForLinkedSocialNetworkId({ linkedSocialNetworkId }) {
      const raw = `select member_id as memberId from linked_social_networks where id = ?`;
      console.log(
        ghostBookshelf.knex.raw(raw, [linkedSocialNetworkId]).toQuery()
      );
      const queryResponse = await ghostBookshelf.knex.raw(raw, [
        linkedSocialNetworkId
      ]);
      console.log("queryResponse getMemberIdForLinkedSocialNetworkId");
      console.log(queryResponse);
      return {
        memberId: queryResponse?.[0]?.[0]?.memberId
      };
    ...
    async getMemberLinkedSocialNetworks({ memberId }) {
      const raw = `select * from linked_social_networks where member_id = ?`;
      console.log(ghostBookshelf.knex.raw(raw, [memberId]).toQuery());
      const queryResponse = await ghostBookshelf.knex.raw(raw, [memberId]);
      console.log("queryResponse getMemberLinkedSocialNetworks");
      console.log(queryResponse);
