...
class MembersSSR {
   ...
   */
  async _getMemberDataFromToken(token, reqIP) {
    const api = await this._getMembersApi();
    return api.getMemberDataFromMagicLinkToken(token, reqIP);
  }
  ...
  async exchangeTokenForSession(req, res) {
    ...
    const member = await this._getMemberDataFromToken(token, req.ip);
    ...
    if (member.customError) {
      return Promise.reject(
        new BadRequestError({
          message: member.customError,
          property: "auth"
        })
      );
    }
